...
namespace deffinumreport_interactions;

defined('MOODLE_INTERNAL') || die();

require_once($CFG->dirroot.'/mod/deffinum/report/interactions/responsessettings_form.php');
...
class report extends \mod_deffinum\report {
    ...
    public function display($deffinum, $cm, $course, $download) {
        ...
        // Detailed report.
        $mform = new \mod_deffinum_report_interactions_settings($PAGE->url, compact('currentgroup'));
        if ($fromform = $mform->get_data()) {
            ...
            set_user_preference('deffinum_report_interactions_qtext', $includeqtext);
            set_user_preference('deffinum_report_interactions_resp', $includeresp);
            set_user_preference('deffinum_report_interactions_right', $includeright);
            set_user_preference('deffinum_report_interactions_result', $includeresult);
        } else {
            $pagesize = get_user_preferences('deffinum_report_pagesize', 0);
            $includeqtext = get_user_preferences('deffinum_report_interactions_qtext', 0);
            $includeresp = get_user_preferences('deffinum_report_interactions_resp', 1);
            $includeright = get_user_preferences('deffinum_report_interactions_right', 0);
            $includeresult = get_user_preferences('deffinum_report_interactions_result', 0);
        }
        if ($pagesize < 1) {
        ...
        if ( !$nostudents ) {
            ...
            if ($download) {
                $filename = clean_filename("$course->shortname ".format_string($deffinum->name, true, $formattextoptions));
            }
            ...
            $scoes = $DB->get_records('deffinum_scoes', array("deffinum" => $deffinum->id), 'sortorder, id');
...
//                }
            ...
            $countsql .= $from.$where;
            $questioncount = get_deffinum_question_count($deffinum->id);
            $nbmaincolumns = count($columns);
            for ($id = 0; $id < $questioncount; $id++) {
                if ($displayoptions['qtext']) {
                    $columns[] = 'question' . $id;
                    $headers[] = get_string('questionx', 'deffinumreport_interactions', $id);
                }
                if ($displayoptions['resp']) {
                    $columns[] = 'response' . $id;
                    $headers[] = get_string('responsex', 'deffinumreport_interactions', $id);
                }
                if ($displayoptions['right']) {
                    $columns[] = 'right' . $id;
                    $headers[] = get_string('rightanswerx', 'deffinumreport_interactions', $id);
                }
                if ($displayoptions['result']) {
                    $columns[] = 'result' . $id;
                    $headers[] = get_string('resultx', 'deffinumreport_interactions', $id);
                }
            }
            ...
            if ($attempts) {
                foreach ($attempts as $scouser) {
                    ...
                    if (empty($timetracks->start)) {
                        ...
                        if (!$download) {
                            $url = new \moodle_url('/mod/deffinum/report/userreport.php',
                                array('id' => $cm->id,
                                    'user' => $scouser->userid,
