 ...
 *
 * @package    deffinumreport_graphs
 * @copyright  2012 Ankit Kumar Agarwal
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace deffinumreport_graphs;
...
class report extends \mod_deffinum\report {
    ...
    protected function get_data($scoid, $allowedlistsql) {
        ...
        foreach ($attempts as $attempt) {
            if ($trackdata = deffinum_get_tracks($scoid, $attempt->userid, $attempt->attempt)) {
    ...
    public function display($deffinum, $cm, $course, $download) {
        ...
        $allowedlistsql = get_enrolled_sql($contextmodule, 'mod/deffinum:savetrack', (int) $group);

        // Labels.
        $labels = [get_string('invaliddata', 'deffinumreport_graphs')];
        ...
        if ($scoes = $DB->get_records('deffinum_scoes', array("deffinum" => $deffinum->id), 'sortorder, id')) {
            foreach ($scoes as $sco) {
                if ($sco->launch != '') {
                    ...
                    $chart->get_xaxis(0, true)->set_label(get_string('percent', 'deffinumreport_graphs'));
                    $yaxis = $chart->get_yaxis(0, true);
                    $yaxis->set_label(get_string('participants', 'deffinumreport_graphs'));
