...
$mode = required_param('mode', PARAM_ALPHA); // Deffinum mode.
$attempt = optional_param('attempt', 1, PARAM_INT); // attempt number.
$download = optional_param('download', '', PARAM_ALPHA);

// Building the url to use for links.+ data details buildup.
$url = new moodle_url('/mod/deffinum/report/userreporttracks.php', array('id' => $id,
    'user' => $userid,
    'attempt' => $attempt,
    'scoid' => $scoid,
    'mode' => $mode));
...
$selsco = $DB->get_record('deffinum_scoes', array('id' => $scoid), '*', MUST_EXIST);
...
$event = \mod_deffinum\event\tracks_viewed::create(array(
    'context' => $contextmodule,
    'relateduserid' => $userid,
    'other' => array('attemptid' => $attempt, 'instanceid' => $deffinum->id, 'scoid' => $scoid, 'mode' => $mode)
));
...
$PAGE->navbar->add("$strattempt $attempt - ".fullname($user),
    new moodle_url('/mod/deffinum/report/userreport.php', array('id' => $id, 'user' => $userid, 'attempt' => $attempt)));
$PAGE->navbar->add($selsco->title . ' - '. get_string('details', 'deffinum'));
$PAGE->set_secondary_active_tab('deffinumreport');

if ($trackdata = deffinum_get_tracks($selsco->id, $userid, $attempt)) {
    if ($trackdata->status == '') {
...
$exportfilename = $courseshortname . '-' . format_string($deffinum->name, true) . '-' . get_string('details', 'deffinum');

$table = new flexible_table('mod_deffinum-userreporttracks');
...
if (!$table->is_downloading($download, $exportfilename)) {
    ...
    $useractionreport = new \mod_deffinum\output\userreportsactionbar($id, $userid, $attempt, 'attempt', $mode, $scoid);
    echo $renderer->user_report_actionbar($useractionreport);
    echo $OUTPUT->box_start('generalbox boxaligncenter');
    echo $OUTPUT->heading("$strattempt $attempt - ". fullname($user).': '.
    format_string($selsco->title). ' - '. get_string('details', 'deffinum'), 3);
}
$table->define_baseurl($PAGE->url);
$table->define_columns(array('element', 'value'));
$table->define_headers(array(get_string('element', 'deffinum'), get_string('value', 'deffinum')));
$table->set_attribute('class', 'generaltable generalbox boxaligncenter deffinumtrackreport');
$table->show_download_buttons_at(array(TABLE_P_BOTTOM));
$table->setup();

foreach ($trackdata as $element => $value) {
    // BEGIN DEFFINUM CUSTOMIZATION.
    // Define elements to exclude: standard fields or non-custom entries.
    $excludedelements = [
        'score_raw',
        'status',
        'total_time',
    ...
    if (substr($element, 0, 3) == 'cmi') {
        ...
        if (stristr($element, '.id') !== false) {
            ...
            $string = "trackresult";
        } else if (stristr($element, '.student_response') !== false or // DEFFINUM 1.2 value.
            stristr($element, '.learner_response') !== false) { // DEFFINUM 2004 value.
        ...
        if (empty($string) || $table->is_downloading()) {
            ...
            $row[] = s($element) . $OUTPUT->help_icon($string, 'deffinum');
        }
        if (strpos($element, '_time') === false) {
            $row[] = s($value);
        } else {
            $row[] = s(deffinum_format_duration($value));
        }
        $table->add_data($row);
    // BEGIN DEFFINUM CUSTOMIZATION.
    } else if (!in_array($element, $excludedelements)) {
        // If the element is not in the excluded list, process it.
        $row = [];
        $row[] = s($element); // Add the element name (escaped for security).
        $row[] = s($value); // Add the corresponding value (escaped for security).
        $table->add_data($row); // Add the row to the table for display.
    }
    // END DEFFINUM CUSTOMIZATION.
}
$table->finish_output();
