...
$url = new moodle_url('/mod/deffinum/report/userreportinteractions.php', array('id' => $id,
    'user' => $userid,
    'attempt' => $attempt));
...
$event = \mod_deffinum\event\interactions_viewed::create(array(
...
$sql = "SELECT a.id, a.userid, a.deffinumid, v.scoid, a.attempt, v.value, v.timemodified, e.element
...
$trackdata = $DB->get_records_sql($sql, ['userid' => $userid, 'deffinumid' => $deffinum->id, 'attempt' => $attempt]);
$usertrack = deffinum_format_interactions($trackdata);

$questioncount = get_deffinum_question_count($deffinum->id);

$courseshortname = format_string($course->shortname, true,
    array('context' => context_course::instance($course->id)));
$exportfilename = $courseshortname . '-' . format_string($deffinum->name, true) . '-' . get_string('interactions', 'deffinum');


// Set up the table.
$table = new flexible_table('mod-deffinum-userreport-interactions');
if (!$table->is_downloading($download, $exportfilename)) {
    ...
    $useractionreport = new \mod_deffinum\output\userreportsactionbar($id, $userid, $attempt, 'interact', $mode);
...
$table->define_headers(array(get_string('trackid', 'deffinum'), get_string('response', 'deffinum'),
    get_string('rightanswer', 'deffinum'), get_string('result', 'deffinum'),
    get_string('calculatedweight', 'deffinum')));
