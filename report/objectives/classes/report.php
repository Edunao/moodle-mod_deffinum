...
/**
 * Core Report class of objectives DEFFINUM report plugin
...
namespace deffinumreport_objectives;

defined('MOODLE_INTERNAL') || die();

require_once($CFG->dirroot.'/mod/deffinum/report/objectives/responsessettings_form.php');
...
class report extends \mod_deffinum\report {
    ...
    public function display($deffinum, $cm, $course, $download) {
        ...
        // Detailed report.
        $mform = new \mod_deffinum_report_objectives_settings($PAGE->url, compact('currentgroup'));
        if ($fromform = $mform->get_data()) {
            $pagesize = $fromform->pagesize;
            $showobjectivescore = $fromform->objectivescore;
            set_user_preference('deffinum_report_pagesize', $pagesize);
            set_user_preference('deffinum_report_objectives_score', $showobjectivescore);
        } else {
            $pagesize = get_user_preferences('deffinum_report_pagesize', 0);
            $showobjectivescore = get_user_preferences('deffinum_report_objectives_score', 0);
        }
        if ($pagesize < 1) {
        ...
        if ( !$nostudents ) {
            ...
            $objectives = get_deffinum_objectives($deffinum->id);
            $nosort = array();
            foreach ($objectives as $scoid => $sco) {
                foreach ($sco as $id => $objectivename) {
                    ...
                    if (!$displayoptions['objectivescore']) {
                        ...
                        $headers[] = $objectivename. ' '. get_string('status', 'deffinumreport_objectives');

                        // Now print objective score headers.
                        $colid = $scoid.'objectivescore' . $id;
                        $columns[] = $colid;
                        $nosort[] = $colid;
                        $headers[] = $objectivename. ' '. get_string('score', 'deffinumreport_objectives');
                    }
                }
            }
...
/**
 * Returns The maximum numbers of Objectives associated with an Deffinum Pack
 *
 * @param int $deffinumid Deffinum instance id
 * @return array an array of possible objectives.
 */
function get_deffinum_objectives($deffinumid) {
    global $DB;
    $objectives = [];
    $params = ['deffinumid' => $deffinumid, 'search' => 'cmi.objectives%.id'];

    $value = $DB->sql_compare_text('v.value');
    $sql = "SELECT DISTINCT $value as value, ss.id
              FROM {deffinum_scoes_value} v
              JOIN {deffinum_scoes} ss ON ss.id = v.scoid AND ss.deffinum = :deffinumid
              JOIN {deffinum_element} e ON v.elementid = e.id
