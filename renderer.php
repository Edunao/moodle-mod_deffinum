...
/**
 * Defines the renderer for the deffinum module.
 *
...
/**
 * The renderer for the deffinum module.
 *
 * @copyright 2013 Dan Marsden
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_deffinum_renderer extends plugin_renderer_base {
    public function view_user_heading($user, $course, $baseurl, $attempt, $attemptids) {
        ...
        $pb = new mod_deffinum_attempt_bar($attemptids, $attempt, $baseurl, 'attempt');
    ...
    /**
     * deffinum attempt bar renderer
     *
     * @param mod_deffinum_attempt_bar $attemptbar
     * @return string
     */
    protected function render_mod_deffinum_attempt_bar(mod_deffinum_attempt_bar $attemptbar) {
        $output = '';
        $attemptbar = clone($attemptbar);
        $attemptbar->prepare($this, $this->page, $this->target);

        if (count($attemptbar->attemptids) > 1) {
            $output .= get_string('attempt', 'deffinum') . ':';
     ...
     *
     * @param \mod_deffinum\output\actionbar $actionbar actionbar object.
     * @return bool|string rendered HTML for the report action.
     */
    public function report_actionbar(\mod_deffinum\output\actionbar $actionbar): string {
        return $this->render_from_template('mod_deffinum/report_actionbar', $actionbar->export_for_template($this));
    }

    /**
     * Rendered HTML for the user report action is provided
     *
     * @param \mod_deffinum\output\userreportsactionbar $userreportsactionbar userreportsactionbar object
     * @return string rendered HTML for the user report action.
     */
    public function user_report_actionbar(\mod_deffinum\output\userreportsactionbar $userreportsactionbar): string {
        return $this->render_from_template('mod_deffinum/user_report_actionbar', $userreportsactionbar->export_for_template($this));
    }

    /**
     * Generate the DEFFINUM's "Exit activity" button
     *
     * @param string $url The url to be hooked up to the exit button
     * @return string
     */
    public function generate_exitbar(string $url): string {
        return $this->render_from_template('mod_deffinum/player_exitbar', ['action' => $url]);
    }
}

/**
 * Component representing a DEFFINUM attempts bar.
 *
 * @copyright 2013 Dan Marsden
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @package mod_deffinum
 */
class mod_deffinum_attempt_bar implements renderable {
    ...
    /**
     * Constructor mod_deffinum_attempt_bar with only the required params.
     *
    ...
    /**
     * Prepares the deffinum attempt bar for output.
     *
     * This method validates the arguments set up for the deffinum attempt bar and then
    ...
    public function prepare(renderer_base $output, moodle_page $page, $target) {
        if (empty($this->attemptids)) {
            throw new coding_exception('mod_deffinum_attempt_bar requires a attemptids value.');
        }
        if (!isset($this->attempt) || is_null($this->attempt)) {
            throw new coding_exception('mod_deffinum_attempt_bar requires a attempt value.');
        }
        if (empty($this->baseurl)) {
            throw new coding_exception('mod_deffinum_attempt_bar requires a baseurl value.');
        }
