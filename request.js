...
function DoRequest(httpReq, url, param, allowBeaconAPI) {
    ...
    var useBeaconAPI = function() {
        if (typeof window.mod_deffinum_useBeaconAPI === 'undefined' || window.mod_deffinum_useBeaconAPI === false) {
            // Last ditch effort, the DEFFINUM package may have introduced its own listeners before our listeners.
            ...
            if (window.event && ['beforeunload', 'unload', 'pagehide'].indexOf(window.event.type)) {
                window.mod_deffinum_useBeaconAPI = true;
            }
        }
        return (window.mod_deffinum_useBeaconAPI && canUseBeaconAPI());
    };
    ...
    var useSendBeacon = function(url, param) {
        ...
        if (!outcome) {
            if (console && console.log) {
                console.log('mod_deffinum: Failed to queue navigator.sendBeacon request');
            }
            return "false\n101";
        }
    ...
    try {
        ...
        if (console && console.log) {
            ...
            var message = 'XHR request from mod_deffinum::DoRequest failed';
            if (canUseBeaconAPI()) {
 ...
 */
window.mod_deffinum_useBeaconAPI = false;
 ...
 *  - visibilitychange
 */
function mod_deffinum_monitorForBeaconRequirement(target) {

    if (typeof target.mod_deffinum_monitoring_for_beacon_requirement !== 'undefined') {
        // We're already observing unload events on this target.
        console.log('mod_deffinum: unload event handlers already attached');
        return;
    }
    target.mod_deffinum_monitoring_for_beacon_requirement = true;
    ...
    // This unfortunately is not ideal, but is required as a DEFFINUM package may also be listening to this and
...
mod_deffinum_monitorForBeaconRequirement(window);