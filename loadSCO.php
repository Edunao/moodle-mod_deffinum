...
if (!empty($id)) {
    if (! $cm = get_coursemodule_from_id('deffinum', $id)) {
    ...
    if (! $cm = get_coursemodule_from_instance('deffinum', $deffinum->id, $course->id)) {
        throw new \moodle_exception('invalidcoursemodule');
    }
} else {
...
$PAGE->set_url('/mod/deffinum/loadSCO.php', array('scoid' => $scoid, 'id' => $cm->id));
...
// Check if DEFFINUM is available.
deffinum_require_available($deffinum);

$context = context_module::instance($cm->id);

// Forge SCO URL.
list($sco, $scolaunchurl) = deffinum_get_sco_and_launch_url($deffinum, $scoid, $context);

if ($sco->deffinumtype == 'asset') {
    $attempt = deffinum_get_last_attempt($deffinum->id, $USER->id);
    $element = (deffinum_version_check($deffinum->version, DEFFINUM_13)) ? 'cmi.completion_status' : 'cmi.core.lesson_status';
    $value = 'completed';
    deffinum_insert_track($USER->id, $deffinum->id, $sco->id, $attempt, $element, $value);
}
...
if ($sco->deffinumtype == 'asset') {
    ...
    echo html_writer::start_tag('html');
    echo html_writer::tag('body', html_writer::tag('p', get_string('activitypleasewait', 'deffinum')));
    echo html_writer::end_tag('html');
    exit;
}
...
?>
   ...
   function doredirect() {
        if (myGetAPIHandle() != null) {
            ...
            document.body.innerHTML = "<p><?php echo get_string('activityloading', 'deffinum');?>" +
                                        "<span id='countdown'><?php echo $delayseconds ?></span> " +
                                        "<?php echo get_string('numseconds', 'moodle', '');?>. &nbsp; " +
                                        "<?php echo addslashes($OUTPUT->pix_icon('wait', '', 'deffinum')); ?></p>";
            ...
            var timer = setInterval(function() {
                                            if( cSeconds && myGetAPIHandle() == null ) {
                                                ...
                                                clearInterval(timer);
                                                document.body.innerHTML = "<p><?php echo get_string('activitypleasewait', 'deffinum');?></p>";
                                                location = "<?php echo $scolaunchurl ?>";
                                            }
                                        }, 1000);
        }
...
echo html_writer::tag('body', html_writer::tag('p', get_string('activitypleasewait', 'deffinum')), array('onload' => "doredirect();"));
echo html_writer::end_tag('html');
