...
/**
 * Define all the backup steps that will be used by the backup_deffinum_activity_task
 */

/**
 * Define the complete deffinum structure for backup, with file and id annotations
 */
class backup_deffinum_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        // ADDED customtype and customdata.
        $deffinum = new backup_nested_element('deffinum', array('id'), array(
            'name', 'deffinumtype', 'customtype', 'customdata', 'reference', 'intro',
        ...
        $sco = new backup_nested_element('sco', array('id'), array(
            ...
            'launch', 'deffinumtype', 'title', 'sortorder'));
        ...
        // Define sources
        $deffinum->set_source_table('deffinum', array('id' => backup::VAR_ACTIVITYID));

        // Order is important for several DEFFINUM calls (especially deffinum_scoes) in the following calls to set_source_table
        $sco->set_source_table('deffinum_scoes', array('deffinum' => backup::VAR_PARENTID), 'sortorder, id');
        $scodata->set_source_table('deffinum_scoes_data', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        $seqrulecond->set_source_table('deffinum_seq_ruleconds', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        $seqrulecondsdata->set_source_table('deffinum_seq_rulecond', array('ruleconditionsid' => backup::VAR_PARENTID), 'id ASC');
        $seqrolluprule->set_source_table('deffinum_seq_rolluprule', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        $seqrolluprulecond->set_source_table('deffinum_seq_rolluprulecond', array('rollupruleid' => backup::VAR_PARENTID), 'id ASC');
        $seqobjective->set_source_table('deffinum_seq_objective', array('scoid' => backup::VAR_PARENTID), 'id ASC');
        $seqmapinfo->set_source_table('deffinum_seq_mapinfo', array('objectiveid' => backup::VAR_PARENTID), 'id ASC');
        ...
        $deffinum->annotate_files('mod_deffinum', 'intro', null); // This file area hasn't itemid
        $deffinum->annotate_files('mod_deffinum', 'content', null); // This file area hasn't itemid
        $deffinum->annotate_files('mod_deffinum', 'package', null); // This file area hasn't itemid
        // BEGIN DEFFINUM CUSTOMIZATION.
        $deffinum->annotate_files('mod_deffinum', 'resource', null); // This file area hasn't itemid
        // END DEFFINUM CUSTOMIZATION.

        // Return the root element (deffinum), wrapped into standard activity structure
        return $this->prepare_activity_structure($deffinum);
    }
}
