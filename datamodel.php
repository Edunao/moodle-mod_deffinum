...
$PAGE->set_url('/mod/deffinum/datamodel.php', array('scoid' => $scoid, 'attempt' => $attempt, 'id' => $cm->id));
...
if (confirm_sesskey() && (!empty($scoid))) {
    ...
    if (has_capability('mod/deffinum:savetrack', context_module::instance($cm->id))) {
        ...
        $sql = "SELECT e.element, v.value, v.timemodified, v.id as valueid
                  ...
                  WHERE a.deffinumid = :deffinumid AND a.userid = :userid AND v.scoid = :scoid AND a.attempt = :attempt";
        $trackdata = $DB->get_records_sql($sql, ['userid' => $USER->id, 'deffinumid' => $deffinum->id,
                                                 'scoid' => $scoid, 'attempt' => $attempt]);
        $attemptobject = deffinum_get_attempt($USER->id, $deffinum->id, $attempt);
        foreach (data_submitted() as $element => $value) {
            $element = str_replace('__', '.', $element);
            if (substr($element, 0, 3) == 'cmi') {
                $netelement = preg_replace('/\.N(\d+)\./', "\.\$1\.", $element);
                $result = deffinum_insert_track($USER->id, $deffinum->id, $scoid, $attemptobject, $element, $value,
                                             $deffinum->forcecompleted, $trackdata) && $result;
            }
            if (substr($element, 0, 15) == 'adl.nav.request') {
                // DEFFINUM 2004 Sequencing Request.
                ...
                if ($action != $value) {
                    ...
                    $valid = deffinum_seq_overall ($scoid, $USER->id, $action, $attempt);
                    $valid = 'true';
