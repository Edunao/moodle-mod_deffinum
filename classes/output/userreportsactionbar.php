...
class userreportsactionbar implements renderable, templatable {
     ...
     * @param int|null $scoid The deffinum id.
     */
    ...
    public function export_for_template(renderer_base $output): array {
        $data = [
            'backurl' => (new moodle_url('/mod/deffinum/report.php', ['id' => $this->id, 'mode' => $this->mode]))->out(false)
        ];

        if (!$this->scoid) {
            $learnobjects = new moodle_url('/mod/deffinum/report/userreport.php',
                    ['id' => $this->id, 'user' => $this->userid, 'attempt' => $this->attempt, 'mode' => $this->mode]);
            $interactions = new moodle_url('/mod/deffinum/report/userreportinteractions.php',
                    ['id' => $this->id, 'user' => $this->userid, 'attempt' => $this->attempt, 'mode' => $this->mode]);

            $reportmenu[$learnobjects->out(false)] = get_string('scoes', 'deffinum');
            $reportmenu[$interactions->out(false)] = get_string('interactions', 'deffinum');
            ...
            $urlselect = new url_select($reportmenu, $userreporturl, [], 'userdeffinumreport');
