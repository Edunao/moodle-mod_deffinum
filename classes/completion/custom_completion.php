...
namespace mod_deffinum\completion;
...
/**
 * Activity custom completion subclass for the deffinum activity.
 *
 * Contains the class for defining mod_deffinum's custom completion rules
 * and fetching a deffinum instance's completion statuses for a user.
 *
...
class custom_completion extends activity_custom_completion {
    ...
    public function get_state(string $rule): int {
        ...
        switch ($rule) {
            case 'completionstatusrequired':
                ...
                $statuses = array_flip(\deffinum_status_options());
            ...
            case 'completionstatusallscos':
                ...
                $scos = $DB->get_records('deffinum_scoes', ['deffinum' => $this->cm->instance, 'deffinumtype' => 'sco']);
        ...
        if ($status === COMPLETION_INCOMPLETE) {
            $deffinum = $DB->get_record('deffinum', ['id' => $this->cm->instance]);
            $attemptcount = deffinum_get_attempt_count($this->userid, $deffinum);

            if ($deffinum->maxattempt > 0 && $attemptcount >= $deffinum->maxattempt) {
    ...
    public function get_custom_rule_descriptions(): array {
        ...
        $statusstrings = \deffinum_status_options();
        ...
        return [
            'completionstatusrequired' => get_string("completiondetail:completionstatus{$statusrequired}", 'deffinum'),
            'completionscorerequired' => get_string('completiondetail:completionscore', 'deffinum', $scorerequired),
            'completionstatusallscos' => get_string('completiondetail:allscos', 'deffinum'),
        ];
    }
