...
/*******************************************************************/
// Default class for Deffinum plugins
//
// Doesn't do anything on it's own -- it needs to be extended.
// This class displays deffinum reports. Because it is called from
// within /mod/deffinum/report.php you can assume that the page header
...
class report {
    ...
    protected function generate_master_checkbox(): string {
        ...
        $selectalltext = get_string('selectall', 'deffinum');
        $deselectalltext = get_string('selectnone', 'deffinum');
        $mastercheckbox = new \core\output\checkbox_toggleall('deffinum-attempts', true, [
            'name' => 'deffinum-selectall-attempts',
            'value' => 1,
            'label' => $selectalltext,
    ...
    protected function generate_row_checkbox(string $name, string $value): string {
        ...
        $checkbox = new \core\output\checkbox_toggleall('deffinum-attempts', false, [
            'name' => $name,
            'value' => $value,
        ]);
    ...
    protected function generate_delete_selected_button(): string {
        $deleteselectedparams = array(
            'type' => 'submit',
            'value' => get_string('deleteselected', 'deffinum'),
            'class' => 'btn btn-secondary',
            'data-action' => 'toggle',
            'data-togglegroup' => 'deffinum-attempts',
            'data-toggle' => 'action',
            'disabled' => true
        );
