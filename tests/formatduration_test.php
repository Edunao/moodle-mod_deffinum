...
global $CFG;
require_once($CFG->dirroot . '/mod/deffinum/locallib.php'); // Include the code to test.

/**
 * Unit tests for deffinum_formatduration function from locallib.php
 *
...
final class formatduration_test extends \basic_testcase {
    public function test_deffinum2004_format(): void {
        ...
        foreach ($suts as $key => $sut) {
            $formatted = deffinum_format_duration($sut);
            $this->assertEquals($formatted, $validates[$key]);
        }
    }

    public function test_deffinum12_format(): void {
        $stryears = get_string('years');
        $strmonths = trim(get_string('nummonths'));
        $strdays = get_string('days');
        $strhours = get_string('hours');
