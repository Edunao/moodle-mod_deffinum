...
/**
 * This file contains tests for deffinum events.
 *
...
/**
 * Test class for various events related to Deffinum.
 *
 * @package    mod_deffinum
 * @copyright  2013 onwards Ankit Agarwal
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
final class events_test extends \advanced_testcase {
    ...
    /** @var stdClass store deffinum object */
    protected $eventdeffinum;
    ...
    protected function setUp(): void {
        ...
        $this->eventdeffinum = $this->getDataGenerator()->create_module('deffinum', $record);
        $this->eventcm = get_coursemodule_from_instance('deffinum', $this->eventdeffinum->id);
    }
    ...
    public function test_attempt_deleted_event(): void {
        ...
        $this->resetAfterTest();
        deffinum_insert_track(2, $this->eventdeffinum->id, 1, 4, 'cmi.core.score.raw', 10);
        $sink = $this->redirectEvents();
        deffinum_delete_attempt(2, $this->eventdeffinum, 4);
        ...
        $this->assertInstanceOf('\mod_deffinum\event\attempt_deleted', $event);
        ...
        \mod_deffinum\event\attempt_deleted::create(array(
            'contextid' => 5,
            'relateduserid' => 2
        ));
    }
    ...
    public function test_interactions_viewed_event_validations(): void {
        $this->resetAfterTest();
        try {
            \mod_deffinum\event\interactions_viewed::create(array(
                'context' => \context_module::instance($this->eventcm->id),
                'courseid' => $this->eventcourse->id,
                'other' => array('attemptid' => 2)
            ));
            $this->fail("Event validation should not allow \\mod_deffinum\\event\\interactions_viewed to be triggered without
                    other['instanceid']");
        } catch (\Exception $e) {
    ...
    public function test_tracks_viewed_event_validations(): void {
        $this->resetAfterTest();
        try {
            \mod_deffinum\event\tracks_viewed::create(array(
                'context' => \context_module::instance($this->eventcm->id),
                'courseid' => $this->eventcourse->id,
                'other' => array('attemptid' => 2, 'scoid' => 2)
            ));
            $this->fail("Event validation should not allow \\mod_deffinum\\event\\tracks_viewed to be triggered without
                    other['instanceid']");
        } catch (\Exception $e) {
            $this->assertInstanceOf('coding_exception', $e);
        }
    ...
    public function test_user_report_viewed_event_validations(): void {
        $this->resetAfterTest();
        try {
            \mod_deffinum\event\user_report_viewed::create(array(
                'context' => \context_module::instance($this->eventcm->id),
                'courseid' => $this->eventcourse->id,
                'other' => array('attemptid' => 2)
            ));
            $this->fail("Event validation should not allow \\mod_deffinum\\event\\user_report_viewed to be triggered without
                    other['instanceid']");
        } catch (\Exception $e) {
            $this->assertInstanceOf('coding_exception', $e);
        }
    ...
    public static function get_scoreraw_submitted_event_validations(): array {
        return array(
            'scoreraw_submitted => missing cmielement' => array(
                null, '50',
                "Event validation should not allow \\mod_deffinum\\event\\scoreraw_submitted " .
    ...
    public function test_scoreraw_submitted_event_validations($cmielement, $cmivalue, $failmessage, $excmessage): void {
        ...
        try {
            ...
            \mod_deffinum\event\scoreraw_submitted::create($data);
    ...
    public static function get_status_submitted_event_validations(): array {
        return array(
            'status_submitted => missing cmielement' => array(
                null, 'passed',
                "Event validation should not allow \\mod_deffinum\\event\\status_submitted " .
                    "to be triggered without other['cmielement']",
                'Coding error detected, it must be fixed by a programmer: ' .
                    "The 'cmielement' must be set in other."
            ),
    ...
    public function test_status_submitted_event_validations($cmielement, $cmivalue, $failmessage, $excmessage): void {
        ...
        try {
            ...
            \mod_deffinum\event\status_submitted::create($data);
            $this->fail($failmessage);
        } catch (\Exception $e) {
            $this->assertInstanceOf('coding_exception', $e);
            $this->assertEquals($excmessage, $e->getMessage());
