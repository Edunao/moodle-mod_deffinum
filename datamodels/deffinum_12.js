...
//
// DEFFINUM 1.2 API Implementation
//
function DEFFINUMapi1_2(def, cmiobj, cmiint, cmistring256, cmistring4096, deffinumdebugging, deffinumauto, deffinumid, cfgwwwroot, sesskey,
    ...
    function LMSInitialize (param) {
        scoid = (deffinum_current_node && deffinum_current_node.scoid) ?  deffinum_current_node.scoid : scoid;
        initdatamodel(scoid);
                ...
                if (typeof datamodel[scoid][elementmodel] != "undefined") {
                    if (datamodel[scoid][elementmodel].mod != 'r') {
                        ...
                        if (matches != null) {
                            ...
                            //Store data
                            if (errorCode == "0") {
                                if (autocommit && !(DEFFINUMapi1_2.timeout)) {
                                    DEFFINUMapi1_2.timeout = Y.later(60000, API, 'LMSCommit', [""], false);
                                }
    ...
    function LMSCommit (param) {
        if (DEFFINUMapi1_2.timeout) {
            DEFFINUMapi1_2.timeout.cancel();
            DEFFINUMapi1_2.timeout = null;
        }
        errorCode = "0";
        if (param == "") {
            if (Initialized) {
    ...
    function StoreData(data,storetotaltime) {
        ...
        //alert('going to:' + "<?php p($CFG->wwwroot) ?>/mod/deffinum/datamodel.php" + "id=<?php p($id) ?>&a=<?php p($a) ?>&sesskey=<?php echo sesskey() ?>"+datastring);
...
M.deffinum_api.init = function(Y, def, cmiobj, cmiint, cmistring256, cmistring4096, deffinumdebugging, deffinumauto, deffinumid, cfgwwwroot,
        sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit, masteryoverride, hidetoc) {
    window.API = new DEFFINUMapi1_2(def, cmiobj, cmiint, cmistring256, cmistring4096, deffinumdebugging, deffinumauto, deffinumid, cfgwwwroot,
            sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit, masteryoverride, hidetoc);
}
