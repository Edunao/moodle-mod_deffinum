...
function deffinum_seq_exit_action_rules($seq, $userid) {
    $sco = $seq->currentactivity;
    $ancestors = deffinum_get_ancestors($sco);
    $exittarget = null;
    foreach (array_reverse($ancestors) as $ancestor) {
        if (deffinum_seq_rules_check($ancestor, 'exit') != null) {
            $exittarget = $ancestor;
            break;
        }
    }
    if ($exittarget != null) {
        $commons = array_slice($ancestors, 0, deffinum_find_common_ancestor($ancestors, $exittarget));

        // Terminate Descendent Attempts Process.
        if ($commons) {
            foreach ($commons as $ancestor) {
                deffinum_seq_end_attempt($ancestor, $userid, $seq->attempt);
...
function deffinum_seq_post_cond_rules($seq, $userid) {
    $sco = $seq->currentactivity;
    if (!$seq->suspended) {
        if ($action = deffinum_seq_rules_check($sco, 'post') != null) {
            switch($action) {
                case 'retry':
                case 'continue':
                case 'previous':
...
function deffinum_seq_evaluate_rollupcond($sco, $conditioncombination, $rollupruleconds, $userid) {
    $bag = Array();
    $con = "";
    $val = false;
    $unk = false;
    foreach ($rollupruleconds as $rolluprulecond) {
        $condit = deffinum_evaluate_condition($rolluprulecond, $sco, $userid);
...
function deffinum_seq_check_child ($sco, $action, $userid) {
    global $DB;

    $included = false;
    $sco = deffinum_get_sco($sco->id);
    $r = deffinum_get_sco_value($sco->id, $userid, 'activityattemptcount');
    if ($action == 'satisfied' || $action == 'notsatisfied') {
        if (!$sco->rollupobjectivesatisfied) {
            ...
            if (($action == 'satisfied' && $sco->requiredforsatisfied == 'ifnotsuspended') ||
                ...
                if (!deffinum_seq_is('activityprogressstatus', $sco->id, $userid) ||
                    ((($r->value) > 0) && !deffinum_seq_is('suspended', $sco->id, $userid))) {
                    $included = false;
                }
...
function deffinum_seq_sequencing ($scoid, $userid, $seq) {

    switch ($seq->sequencing) {
        case 'start':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = deffinum_seq_start_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'resumeall':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = deffinum_seq_resume_all_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'exit':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = deffinum_seq_exit_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'retry':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = deffinum_seq_retry_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'previous':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = deffinum_seq_previous_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;

        case 'choice':
            // We'll see the parameters we have to send, this should update delivery and end.
            $seq = deffinum_seq_choice_sequencing($scoid, $userid, $seq);
            $seq->sequencing = true;
            break;
    }
...
function deffinum_seq_start_sequencing($scoid, $userid, $seq) {
    global $DB;
    ...
    $sco = $DB->get_record('deffinum_scoes', array('scoid' => $scoid, 'userid' => $userid));
    if (($sco->parent == '/') && deffinum_is_leaf($sco)) { // If the activity is the root and is leaf.
        $seq->delivery = $sco;
    } else {
        $ancestors = deffinum_get_ancestors($sco);
        $ancestorsroot = array_reverse($ancestors);
        $res = deffinum_seq_flow($ancestorsroot[0], 'forward', $seq, true, $userid);
        if ($res) {
            return $res;
        }
    }
...
function deffinum_seq_resume_all_sequencing($scoid, $userid, $seq) {
    global $DB;
    ...
    $track = deffinum_get_sco_value($scoid, $userid, 'suspendedactivity');
    if (!$track) {
        $seq->delivery = null;
    ...
    $seq->delivery = $DB->get_record('deffinum_scoes', array('scoid' => $scoid, 'userid' => $userid));
}

function deffinum_seq_continue_sequencing($scoid, $userid, $seq) {
    if (empty($seq->currentactivity)) {
        $seq->delivery = null;
    ...
    if ($currentact->parent != '/') {
        ...
        $parent = deffinum_get_parent ($currentact);
        ...
        $res = deffinum_seq_flow($currentact, 'forward', $seq, false, $userid);
        if ($res) {
            return $res;
        }
    }
...
function deffinum_seq_previous_sequencing($scoid, $userid, $seq) {
    if (empty($seq->currentactivity)) {
        $seq->delivery = null;
    ...
    if ($currentact->parent != '/') { // If the activity is the root and is leaf.
        ...
        $res = deffinum_seq_flow($currentact, 'backward', $seq, false, $userid);
        if ($res) {
            return $res;
        }
    }
...
function deffinum_seq_exit_sequencing($scoid, $userid, $seq) {
    if (empty($seq->currentactivity)) {
        $seq->delivery = null;
...
function deffinum_seq_retry_sequencing($scoid, $userid, $seq) {
    if (empty($seq->currentactivity)) {
        $seq->delivery = null;
    ...
    if (!deffinum_is_leaf($seq->currentactivity)) {
        $res = deffinum_seq_flow($seq->currentactivity, 'forward', $seq, true, $userid);
        if ($res != null) {
            return $res;
        } else {
            // Return deliver.
...
function deffinum_seq_choice_sequencing($sco, $userid, $seq) {
    ...
    foreach ($arrpath as $activity) {
        if ($activity->parent != '/') {
            $avchildren = deffinum_get_available_children (deffinum_get_parent($activity));
        ...
        if (deffinum_seq_rules_check($activity, 'hidefromchoice' != null)) {
            $seq->delivery = null;
    ...
    if ($sco->parent != '/') {
        $parent = deffinum_sco_get_parent ($sco);
    ...
    if ($seq->currentactivity != null) {
        $commonpos = deffinum_find_common_ancestor($ancestors, $seq->currentactivity);
    ...
    if ($seq->currentactivity === $sco) {
        // MDL-51757 - this part of the DEFFINUM 2004 sequencing and navigation was not completed.
        throw new \coding_exception('Unexpected state encountered');
    }

    $sib = deffinum_get_siblings($seq->currentactivity);
    ...
    if ($pos !== false) {
        ...
        $children = deffinum_get_children (deffinum_get_parent ($sco));
        ...
        foreach ($siblings as $sibling) {
            $seq = deffinum_seq_choice_activity_traversal($sibling, $userid, $seq, $traverse);
            if (!$seq->reachable) {
                $seq->delivery = null;
                return $seq;
            }
    ...
    if ($seq->currentactivity == null || $seq->currentactivity == $comancestor) {
        ...
        foreach ($comtarget as $act) {
            $seq = deffinum_seq_choice_activity_traversal($act, $userid, $seq, 'forward');
            if (!$seq->reachable) {
                $seq->delivery = null;
                return $seq;
            }
            $act = deffinum_get_sco ($acti->id);
            if (deffinum_seq_is('active', $act->id, $userid) && ($act->id != $comancestor->id && $act->preventactivation)) {
                $seq->delivery = null;
    ...
    if ($comancestor->id == $sco->id) {
        ...
        $ancestorscurrent = deffinum_get_ancestors($seq->currentactivity);
    ...
    if (array_search($ancestors, $comancestor) !== false) {
        $ancestorscurrent = deffinum_get_ancestors($seq->currentactivity);
        $commonpos = deffinum_find_common_ancestor($ancestors, $sco);
        ...
        if ($constrained != null) {
            $fwdir = deffinum_get_preorder($constrained);
            ...
            $seq = deffinum_seq_choice_flow($constrained, $traverse, $seq);
            $actconsider = $seq->identifiedactivity;
            $avdescendents = Array();
            $avdescendents = deffinum_get_available_descendents($actconsider);
        ...
        $fwdir = deffinum_get_preorder($seq->currentactivity);
        ...
        if (array_search($fwdir, $sco) !== false) {
            foreach ($comtarget as $act) {
                ...
                $act = deffinum_get_sco($act->id);
                if (deffinum_seq_is('active', $act->id, $userid) && ($act->id != $comancestor->id &&
            ...
            foreach ($comtarget as $act) {
                $act = deffinum_get_sco($act->id);
                if (deffinum_seq_is('active', $act->id, $userid) && $act->id != $comancestor->id && $act->preventactivation == true) {
                    $seq->delivery = null;
                    $seq->exception = 'SB.2.9-6';
                    return $seq;
                }
    ...
    $seq = deffinum_seq_flow($sco, 'forward', $seq, true, $userid);
    if ($seq->deliverable == false) {
        deffinum_terminate_descendent_attempts($comancestor, $userid, $seq);
        deffinum_seq_end_attempt($comancestor, $userid, $seq->attempt);
...
function deffinum_seq_choice_flow ($constrained, $traverse, $seq) {
    $seq = deffinum_seq_choice_flow_tree ($constrained, $traverse, $seq);
...
function deffinum_seq_choice_flow_tree ($constrained, $traverse, $seq) {
    $islast = false;
    $parent = deffinum_get_parent ($constrained);
    if ($traverse == 'forward') {
        $preord = deffinum_get_preorder($constrained);
        ...
        $avchildren = deffinum_get_available_children($parent); // Available children.
        if ($avchildren[count($avchildren) - 1]->id == $constrained->id) {
            $seq = deffinum_seq_choice_flow_tree ($parent, 'forward', $seq);
            return $seq;
        } else {
            $i = 0;
    ...
    if ($traverse == 'backward') {
        ...
        if ($avchildren [0]->id == $constrained->id) {
            $seq = deffinum_seq_choice_flow_tree ($parent, 'backward', $seq);
            return $seq;
        } else {
...
function deffinum_seq_choice_activity_traversal($activity, $userid, $seq, $direction) {
    if ($direction == 'forward') {
        $act = deffinum_seq_rules_check($activity, 'stopforwardtraversal');
    ...
    if ($direction == 'backward') {
        $parentsco = deffinum_get_parent($activity);
        if ($parentsco != null) {
...
function deffinum_sequencing_delivery($scoid, $userid, $seq) {

    if (!deffinum_is_leaf($seq->delivery)) {
        $seq->deliveryvalid = false;
        $seq->exception = 'DB.1.1-1';
        return $seq;
    }
    $ancestors = deffinum_get_ancestors($seq->delivery);
    $arrpath = array_reverse($ancestors);
    ...
    foreach ($arrpath as $activity) {
        if (deffinum_check_activity($activity, $userid)) {
            $seq->deliveryvalid = false;
...
function deffinum_content_delivery_environment($seq, $userid) {
    global $DB;

    $act = $seq->currentactivity;
    if (deffinum_seq_is('active', $act->id, $userid)) {
        $seq->exception = 'DB.2-1';
        return $seq;
    }
    $track = deffinum_get_sco_value($act->id, $userid, 'suspendedactivity');
    if ($track != null) {
        $seq = deffinum_clear_suspended_activity($seq->delivery, $seq, $userid);

    }
    $attemptobject = deffinum_get_attempt($userid, $track->deffinumid, 0);
    $seq = deffinum_terminate_descendent_attempts ($seq->delivery, $userid, $seq);
    $ancestors = deffinum_get_ancestors($seq->delivery);
    $arrpath = array_reverse($ancestors);
    ...
    foreach ($arrpath as $activity) {
        if (!deffinum_seq_is('active', $activity->id, $userid)) {
            if (!isset($activity->tracked) || ($activity->tracked == 1)) {
                if (!deffinum_seq_is('suspended', $activity->id, $userid)) {
                    $r = deffinum_get_sco_value($activity->id, $userid, 'activityattemptcount');
                    $value = new stdClass();
                    $value->id = $r->valueid;
                    $value->value = ($r->value) + 1;
                    $DB->update_record('deffinum_scoes_value', $value);
                    if ($r->value == 1) {
                        deffinum_seq_set('activityprogressstatus', $activity->id, $userid, 'true');
                    }
                    deffinum_insert_track($userid, $activity->deffinum, $activity->id, $attemptobject,
            ...
            deffinum_seq_set('active', $activity->id, $userid, 'true');
        }
    }
    $seq->delivery = $seq->currentactivity;
    deffinum_seq_set('suspendedactivity', $activity->id, $userid, 'false');

    return $seq;
}

function deffinum_clear_suspended_activity($act, $seq, $userid) {
    global $DB;
    $currentact = $seq->currentactivity;
    $track = deffinum_get_sco_value($currentact->id, $userid, 'suspendedactivity');
    if ($track != null) {
        $ancestors = deffinum_get_ancestors($act);
        $commonpos = deffinum_find_common_ancestor($ancestors, $currentact);
        if ($commonpos !== false) {
                ...
                if (!empty($activitypath)) {
                    ...
                    foreach ($activitypath as $activity) {
                        if (deffinum_is_leaf($activity)) {
                            deffinum_seq_set('suspended', $activity->id, $userid, false);
                        } else {
                            $children = deffinum_get_children($activity);
                            $bool = false;
                            foreach ($children as $child) {
                                if (deffinum_seq_is('suspended', $child->id, $userid)) {
                                    $bool = true;
                                }
                            }
                            if (!$bool) {
        ...
        deffinum_seq_set('suspendedactivity', $act->id, $userid, false);
    }
}

function deffinum_select_children_process($scoid, $userid) {
    global $DB;

    $sco = deffinum_get_sco($scoid);
    if (!deffinum_is_leaf($sco)) {
        if (!deffinum_seq_is('suspended', $scoid, $userid) && !deffinum_seq_is('active', $scoid, $userid)) {
            $r = deffinum_get_sco_value($scoid, $userid, 'selectiontiming');
            ...
            switch ($r->value) {
                ...
                case 'once':
                    if (!deffinum_seq_is('activityprogressstatus', $scoid, $userid)) {
                        if (deffinum_seq_is('selectioncountsstatus', $scoid, $userid)) {
                            $childlist = '';
                            $res = deffinum_get_sco_value($scoid, $userid, 'selectioncount');
                            $i = ($res->value) - 1;
                            ...
                            deffinum_seq_set('availablechildren', $scoid, $userid, false);
                            deffinum_seq_set('availablechildren', $scoid, $userid, $clist);
                        }
                    }
                break;
            }
...
function deffinum_randomize_children_process($scoid, $userid) {
    global $DB;

    $sco = deffinum_get_sco($scoid);
    if (!deffinum_is_leaf($sco)) {
        if (!deffinum_seq_is('suspended', $scoid, $userid) && !deffinum_seq_is('active', $scoid, $userid)) {
            $r = deffinum_get_sco_value($scoid, $userid, 'randomizationtiming');
            ...
            switch ($r->value) {
                ...
                case 'once':
                    if (!deffinum_seq_is('activityprogressstatus', $scoid, $userid)) {
                        if (deffinum_seq_is('randomizechildren', $scoid, $userid)) {
                            $childlist = array();
                            $res = deffinum_get_available_children($sco);
                            $i = count($res) - 1;
                            $children = $res->value;
...
function deffinum_terminate_descendent_attempts($activity, $userid, $seq) {
    $ancestors = deffinum_get_ancestors($seq->currentactivity);
    $commonpos = deffinum_find_common_ancestor($ancestors, $activity);
    if ($commonpos !== false) {
        if ($activitypath = array_slice($ancestors, 1, $commonpos - 2)) {
            if (!empty($activitypath)) {
                foreach ($activitypath as $sco) {
                    deffinum_seq_end_attempt($sco, $userid, $seq->attempt);
                }
            }
        }
    }
...
function deffinum_sequencing_exception($seq) {
    global $OUTPUT;
    if ($seq->exception != null) {
