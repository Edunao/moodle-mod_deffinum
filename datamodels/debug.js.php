...
function getLoggingActive () {
    var loggingActive = getCookie('DEFFINUMLoggingActive');
    if (!loggingActive) {
        loggingActive = 'A';
    }
...
function setLoggingActive (flag) {
    new cookie("DEFFINUMLoggingActive", flag, 365, "/").set();
}
...
function toggleLog () {
    if (getLoggingActive() == "A") {
        ...
        setLoggingActive('N');
        logButton.innerHTML = '-><?php echo addslashes_js(get_string('deffinumloggingoff', 'deffinum')); ?>';
    } else {
        setLoggingActive('A');
        AppendToLog("Moodle Logging Activated", 0);
        logButton.innerHTML = '<?php echo addslashes_js(get_string('deffinumloggingon', 'deffinum')); ?>';
...
var debugDEFFINUMVersion = '<?php echo $deffinum->version; ?>';
<?php
$lmsprefix = (deffinum_version_check($deffinum->version, DEFFINUM_12) || empty($deffinum->version)) ? 'LMS' : '';
$lmsapi = (deffinum_version_check($deffinum->version, DEFFINUM_12) || empty($deffinum->version)) ? 'API' : 'API_1484_11';

$lmselements = array();
if (deffinum_version_check($deffinum->version, DEFFINUM_12) || empty($deffinum->version)) {
...
function UpdateLog(s) {
    var s1 = '<html><head><style>\n'
        ...
        + '<\/style>'
        + '<script>\n'
        + 'var LMSVersion = \'<?php echo $deffinum->version; ?>\';\n'
        + ' \n'
        ...
        + '          \' so API calls will fail - did navigate to another DEFFINUM package?\'); \n'
        + '    return false; \n'
        + '} \n'
        + ' \n'
        + 'var saveElement = ""; \n'
        ...
        + '<h1>DEFFINUM Debugging interface</h1>'
        + '<h2>DEFFINUM Version Detected: <?php echo $deffinum->version; ?></h2>'
        + '<input type="hidden" id="mod-deffinum-logstate" name="mod-deffinum-logstate" value="A" \/>'
...
?>
        ...
        + '             name="commitButton">'
        + '            <input type="button"'
        + '             value="<?php echo deffinum_version_check($deffinum->version, DEFFINUM_12) ? 'LMSFinish' : 'Terminate'; ?>()  "'
        + '             onclick="try'
        + '             <?php echo deffinum_version_check($deffinum->version, DEFFINUM_12) ? 'LMSFinish' : 'LMSTerminate'; ?>();"'
        + '             id="finishButton"'
        + '             name="finishButton">'
        + '         </td>'
        + '    </tr>'
        ...
        + '   </table>'
        + '</form>'
        + '<h3 id="mod-deffinum-marker">DEFFINUM API Activity Log<\/h3>';
    ...
    if (logPopUpWindow != 'N' && !logPopUpWindow.closed) {
        ...
        logPopUpWindow = open( '', 'deffinumlogpopupwindow', '' );
        popupdoc = logPopUpWindow.document;
        // Is logging active?
        var marker = safeGetElement(popupdoc, 'mod-deffinum-marker');
        if (marker) {
            popupdoc.body.innerHTML += s;
        } else {
            popupdoc.open();
            ...
            popupdoc.close();
            popupdoc.title = 'DEFFINUM API Activity Log';
            logPopUpWindow.focus();
        }
    }
...
function AppendToLog(s, rc) {
    ...
    s = '<div ' + sStyle + ' id="<?php echo $deffinum->version; ?>">' + now.toGMTString() + ': ' + s + '<\/div>';
    UpdateLog(s);
    // switch colours for a new section of work
    if (s.match(/Commit|Loaded|Initialize|Terminate|Finish|Moodle DEFFINUM|Moodle Logging/)) {
        logRow++;
    }
}
...
function LogAPICall(func, nam, val, rc) {
    ...
    s += ' => ' + String(rc) + "   scoid = " + deffinum_current_node.scoid;
    AppendToLog(s, rc);
<?php
if (deffinum_debugging($deffinum) && ($sco->deffinumtype == 'asset')) {
?>
    hint = 'Item <?php echo $sco->identifier; ?> has been defined as an Asset: it should never call the DEFFINUM API';
...
if (!document.getElementById('mod-deffinum-log-toggle')) {
    var logButton = document.createElement('a');
    logButton.id = 'mod-deffinum-log-toggle';
    ...
    var content = safeGetElement(document, 'deffinumpage');
