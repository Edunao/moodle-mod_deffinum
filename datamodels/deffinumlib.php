...
/**
 * functions used by DEFFINUM 1.2/2004 packages.
 *
...
function deffinum_get_resources($blocks) {
    $resources = array();
...
function deffinum_get_manifest($blocks, $scoes) {
    global $OUTPUT;
        ...
        foreach ($blocks as $block) {
            switch ($block['name']) {
                            ...
                            if ($metadata['name'] == 'SCHEMAVERSION') {
                                if (empty($scoes->version)) {
                                    ...
                                    if (isset($metadata['tagData']) && $isversionset) {
                                        $scoes->version = 'DEFFINUM_'.$matches[count($matches) - 1];
                                    } else {
                                        $isversionset = (preg_match("/^2004 (3rd|4th) Edition$/", $metadata['tagData'], $matches));
                                        if (isset($metadata['tagData']) && $isversionset) {
                                            $scoes->version = 'DEFFINUM_1.3';
                                        } else {
                                            $scoes->version = 'DEFFINUM_1.2';
                                        }
                                    }
                                }
                            }
                ...
                case 'MANIFEST':
                    ...
                    $organization = '';
                    $resources = array();
                    $resources = deffinum_get_resources($block['children']);
                    $scoes = deffinum_get_manifest($block['children'], $scoes);
                    if (empty($scoes->elements) || count($scoes->elements) <= 0) {
                        foreach ($resources as $item => $resource) {
                            if (!empty($resource['HREF'])) {
                                ...
                                $sco->deffinumtype = $resource['ADLCP:DEFFINUMTYPE'];
                ...
                case 'ORGANIZATION':
                    ...
                    $scoes->elements[$manifest][$organization][$identifier]->deffinumtype = '';
                ...
                case 'ITEM':
                    ...
                    if (!isset($block['attrs']['IDENTIFIERREF'])) {
                        $scoes->elements[$manifest][$organization][$identifier]->launch = '';
                        $scoes->elements[$manifest][$organization][$identifier]->deffinumtype = 'asset';
                    } else {
                        ...
                        if (!isset($resources[$idref])) {
                            ...
                            if (empty($resources[$idref]['ADLCP:DEFFINUMTYPE'])) {
                                $resources[$idref]['ADLCP:DEFFINUMTYPE'] = 'asset';
                            }
                            $scoes->elements[$manifest][$organization][$identifier]->deffinumtype = $resources[$idref]['ADLCP:DEFFINUMTYPE'];
                        }
                    }
    ...
    if (!empty($manifestresourcesnotfound)) {
        ...
        echo $OUTPUT->notification(get_string('invalidmanifestresource', 'deffinum').' '. implode(', ', $manifestresourcesnotfound));
    }
    return $scoes;
}

/**
 * Sets up DEFFINUM 1.2/2004 packages using the manifest file.
 ...
 * @return bool
 */
function deffinum_parse_deffinum(&$deffinum, $manifest) {
    global $CFG, $DB;
    ...
    $scoes = new stdClass();
    $scoes->version = '';
    $scoes = deffinum_get_manifest($manifests, $scoes);
    $newscoes = array();
    $sortorder = 0;
    if (!empty($scoes->elements) && is_iterable($scoes->elements)) {
        $olditems = $DB->get_records('deffinum_scoes', array('deffinum' => $deffinum->id));
        foreach ($scoes->elements as $manifest => $organizations) {
            foreach ($organizations as $organization => $items) {
                foreach ($items as $identifier => $item) {
                    $sortorder++;
                    // This new db mngt will support all DEFFINUM future extensions.
                    $newitem = new stdClass();
                    $newitem->deffinum = $deffinum->id;
                    $newitem->manifest = $manifest;
                    $newitem->organization = $organization;
                    $newitem->sortorder = $sortorder;
                    $standarddatas = array('parent', 'identifier', 'launch', 'deffinumtype', 'title');
                    ...
                    if (!empty($olditems) && ($olditemid = deffinum_array_search('identifier', $newitem->identifier, $olditems))) {
                        $newitem->id = $olditemid;
                        // Update the Sco sortorder but keep id so that user tracks are kept against the same ids.
                        $DB->update_record('deffinum_scoes', $newitem);
                        $id = $olditemid;
                        // Remove all old data so we don't duplicate it.
                        $DB->delete_records('deffinum_scoes_data', array('scoid' => $olditemid));
                        $DB->delete_records('deffinum_seq_objective', array('scoid' => $olditemid));
                        $DB->delete_records('deffinum_seq_mapinfo', array('scoid' => $olditemid));
                        $DB->delete_records('deffinum_seq_ruleconds', array('scoid' => $olditemid));
                        $DB->delete_records('deffinum_seq_rulecond', array('scoid' => $olditemid));
                        $DB->delete_records('deffinum_seq_rolluprule', array('scoid' => $olditemid));
                        $DB->delete_records('deffinum_seq_rolluprulecond', array('scoid' => $olditemid));
                        ...
                        $id = $DB->insert_record('deffinum_scoes', $newitem);
                    }
                    // Save this sco in memory so we can use it later.
                    $newscoes[$id] = $newitem;

                    if ($optionaldatas = deffinum_optionals_data($item, $standarddatas)) {
                        $data = new stdClass();
                        $data->scoid = $id;
                        foreach ($optionaldatas as $optionaldata) {
                            if (isset($item->$optionaldata)) {
                                ...
                                $dataid = $DB->insert_record('deffinum_scoes_data', $data);
                            }
                        }
                    }
                    ...
                    if (isset($item->sequencingrules)) {
                        foreach ($item->sequencingrules as $sequencingrule) {
                            ...
                            $ruleid = $DB->insert_record('deffinum_seq_ruleconds', $rule);
                            if (isset($sequencingrule->ruleconditions)) {
                                foreach ($sequencingrule->ruleconditions as $rulecondition) {
                                    ...
                                    $rulecondid = $DB->insert_record('deffinum_seq_rulecond', $rulecond);
                                }
                            }
                        }
                    }
                    ...
                    if (isset($item->rolluprules)) {
                        foreach ($item->rolluprules as $rolluprule) {
                            ...
                            $rollupruleid = $DB->insert_record('deffinum_seq_rolluprule', $rollup);
                            if (isset($rollup->conditions)) {
                                foreach ($rollup->conditions as $condition) {
                                    ...
                                    $conditionid = $DB->insert_record('deffinum_seq_rolluprulecond', $cond);
                                }
                            }
                        }
                    }
                    ...
                    if (isset($item->objectives)) {
                        foreach ($item->objectives as $objective) {
                            ...
                            $objectiveid = $DB->insert_record('deffinum_seq_objective', $obj);
                            if (isset($objective->mapinfos)) {
                                foreach ($objective->mapinfos as $objmapinfo) {
                                    ...
                                    $mapinfoid = $DB->insert_record('deffinum_seq_mapinfo', $mapinfo);
                                }
                            }
                        }
                    }
        ...
        if (!empty($olditems)) {
            foreach ($olditems as $olditem) {
                $DB->delete_records('deffinum_scoes', ['id' => $olditem->id]);
                $DB->delete_records('deffinum_scoes_data', ['scoid' => $olditem->id]);
                deffinum_delete_tracks($deffinum->id, $olditem->id);
                $DB->delete_records('deffinum_seq_objective', ['scoid' => $olditem->id]);
                $DB->delete_records('deffinum_seq_mapinfo', ['scoid' => $olditem->id]);
                $DB->delete_records('deffinum_seq_ruleconds', ['scoid' => $olditem->id]);
                $DB->delete_records('deffinum_seq_rulecond', ['scoid' => $olditem->id]);
                $DB->delete_records('deffinum_seq_rolluprule', ['scoid' => $olditem->id]);
                $DB->delete_records('deffinum_seq_rolluprulecond', ['scoid' => $olditem->id]);
            }
        }
        if (empty($scoes->version)) {
            $scoes->version = 'DEFFINUM_1.2';
        }
        $DB->set_field('deffinum', 'version', $scoes->version, array('id' => $deffinum->id));
        $deffinum->version = $scoes->version;
    }
    $deffinum->launch = 0;
    ...
    if (!empty($defaultorgid) && isset($newscoes[$defaultorgid]) && !empty($newscoes[$defaultorgid]->launch)) {
        ...
        $sqlselect = 'deffinum = ? AND sortorder >= ? AND '.$DB->sql_isnotempty('deffinum_scoes', 'launch', false, true);
        // We use get_records here as we need to pass a limit in the query that works cross db.
        $scoes = $DB->get_records_select('deffinum_scoes', $sqlselect, array($deffinum->id, $firstinorg), 'sortorder', 'id', 0, 1);
        if (!empty($scoes)) {
    ...
    if (empty($deffinum->launch)) {
        ...
        // We use get_records here as we need to pass a limit in the query that works cross db.
        $scoes = $DB->get_records_select('deffinum_scoes', $sqlselect, array($deffinum->id), 'sortorder', 'id', 0, 1);
        if (!empty($scoes)) {
            $sco = reset($scoes); // We only care about the first record - the above query only returns one.
            $deffinum->launch = $sco->id;
        }
...
function deffinum_optionals_data($item, $standarddata) {
    $result = array();
...
function deffinum_is_leaf($sco) {
    global $DB;

    if ($DB->record_exists('deffinum_scoes', array('deffinum' => $sco->deffinum, 'parent' => $sco->identifier))) {
        return false;
    }
    return true;
}

function deffinum_get_parent($sco) {
    global $DB;

    if ($sco->parent != '/') {
        if ($parent = $DB->get_record('deffinum_scoes', array('deffinum' => $sco->deffinum, 'identifier' => $sco->parent))) {
            return deffinum_get_sco($parent->id);
        }
    }
    return null;
}

function deffinum_get_children($sco) {
    global $DB;

    $children = $DB->get_records('deffinum_scoes', array('deffinum' => $sco->deffinum, 'parent' => $sco->identifier), 'sortorder, id');
    if (!empty($children)) {
        return $children;
    }
    return null;
...
function deffinum_get_available_children($sco) {
    global $DB;

    $res = $DB->get_records('deffinum_scoes', array('deffinum' => $sco->deffinum, 'parent' => $sco->identifier), 'sortorder, id');
    if (!$res || $res == null) {
        return false;
    } else {
        foreach ($res as $sco) {
...
function deffinum_get_available_descendent($descend, $sco) {
    if ($sco == null) {
        return $descend;
    } else {
        $avchildren = deffinum_get_available_children($sco);
        foreach ($avchildren as $avchild) {
            array_push($descend, $avchild);
        }
        foreach ($avchildren as $avchild) {
            deffinum_get_available_descendent($descend, $avchild);
        }
    }
}

function deffinum_get_siblings($sco) {
    global $DB;

    if ($siblings = $DB->get_records('deffinum_scoes', array('deffinum' => $sco->deffinum, 'parent' => $sco->parent), 'sortorder, id')) {
...
function deffinum_get_ancestors($sco) {
    $ancestors = array();
    $continue = true;
    while ($continue) {
        $ancestor = deffinum_get_parent($sco);
...
function deffinum_get_preorder(&$preorder = array(), $sco = null) {
    if ($sco != null) {
        array_push($preorder, $sco);
        if ($children = deffinum_get_children($sco)) {
            foreach ($children as $child) {
                deffinum_get_preorder($preorder, $child);
            }
        }
    }
    return $preorder;
...
function deffinum_find_common_ancestor($ancestors, $sco) {
    $pos = deffinum_array_search('identifier', $sco->parent, $ancestors);
    if ($sco->parent != '/') {
        if ($pos === false) {
            return deffinum_find_common_ancestor($ancestors, deffinum_get_parent($sco));
        }
    }
    return $pos;
}
