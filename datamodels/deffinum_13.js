...
// Used need to debug cmi content (if you uncomment this, you must comment the definition inside DEFFINUMapi1_3)
//var cmi = new Object();

//
// DEFFINUM 1.3 API Implementation
//
function DEFFINUMapi1_3(def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, deffinumdebugging, deffinumauto, deffinumid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit) {

    var prerequrl = cfgwwwroot + "/mod/deffinum/prereqs.php?a=" + deffinumid + "&scoid=" + scoid + "&attempt=" + attempt + "&mode=" + viewmode + "&currentorg=" + currentorg + "&sesskey=" + sesskey;
    var datamodelurl = cfgwwwroot + "/mod/deffinum/datamodel.php";
    var datamodelurlparams = "id=" + cmid + "&a=" + deffinumid + "&sesskey=" + sesskey + "&attempt=" + attempt + "&scoid=" + scoid;
    ...
    function initdatamodel(scoid){
        ...
        prerequrl = cfgwwwroot + "/mod/deffinum/prereqs.php?a=" + deffinumid + "&scoid=" + scoid + "&attempt=" + attempt + "&mode=" + viewmode + "&currentorg=" + currentorg + "&sesskey=" + sesskey;
        datamodelurlparams = "id=" + cmid + "&a=" + deffinumid + "&sesskey=" + sesskey + "&attempt=" + attempt + "&scoid=" + scoid;
    ...
    function Initialize (param) {
        scoid = deffinum_current_node ? deffinum_current_node.scoid : scoid;
        initdatamodel(scoid);
                ...
                if ('true' == result) {
                    ...
                    if (adl.nav.request != '_none_') {
                        switch (adl.nav.request) {
                            case 'continue':
                                setTimeout('mod_deffinum_launch_next_sco();',500);
                            break;
                            case 'previous':
                                setTimeout('mod_deffinum_launch_prev_sco();',500);
                            break;
                            case 'choice':
                            break;
                            case 'exit':
                    ...
                    // trigger TOC update
                    var callback = M.mod_deffinum.connectPrereqCallback;
                ...
                if (typeof datamodel[scoid][elementmodel] != "undefined") {
                    if (datamodel[scoid][elementmodel].mod != 'r') {
                        ...
                        if ((matches != null) && ((matches.join('').length > 0) || (value.length == 0))) {
                            ...
                            //Store data
                            if (errorCode == "0") {
                                if (autocommit && !(DEFFINUMapi1_3.timeout)) {
                                    DEFFINUMapi1_3.timeout = Y.later(60000, API_1484_11, 'Commit', [""], false);
                                }
    ...
    function Commit (param) {
        if (DEFFINUMapi1_3.timeout) {
            DEFFINUMapi1_3.timeout.cancel();
            DEFFINUMapi1_3.timeout = null;
        }
        errorCode = "0";
        if (param == "") {
            if ((Initialized) && (!Terminated)) {
...
M.deffinum_api.init = function(Y, def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, deffinumdebugging, deffinumauto, deffinumid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit) {
    window.API_1484_11 = new DEFFINUMapi1_3(def, cmiobj, cmiint, cmicommentsuser, cmicommentslms, deffinumdebugging, deffinumauto, deffinumid, cfgwwwroot, sesskey, scoid, attempt, viewmode, cmid, currentorg, autocommit);
}
