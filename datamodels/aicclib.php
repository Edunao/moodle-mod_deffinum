...
function deffinum_add_time($a, $b) {
    $aes = explode(':', $a);
    $bes = explode(':', $b);
 ...
 * @return mixed
 */
function deffinum_get_aicc_columns($row, $mastername='system_id') {
 ...
 * @return string
 */
function deffinum_forge_cols_regexp($columns, $remodule='(".*")?,') {
    $regexp = '/^';
 ...
 * @return bool
 */
function deffinum_parse_aicc(&$deffinum) {
    global $DB;

    if ($deffinum->deffinumtype == DEFFINUM_TYPE_AICCURL) {
        return deffinum_aicc_generate_simple_sco($deffinum);
    }
    ...
    $files = $fs->get_area_files($context->id, 'mod_deffinum', 'content', 0, 'sortorder, itemid, filepath, filename', false);
    ...
    foreach ($ids as $courseid => $id) {
        ...
        if (isset($id->des)) {
            ...
            $rows = explode("\r\n", $contents);
            $columns = deffinum_get_aicc_columns($rows[0]);
            $regexp = deffinum_forge_cols_regexp($columns->columns);
        ...
        if (isset($id->cst)) {
            ...
            $rows = explode("\r\n", $contents);
            $columns = deffinum_get_aicc_columns($rows[0], 'block');
            $regexp = deffinum_forge_cols_regexp($columns->columns, '(.+)?,');
            for ($i = 1; $i < count($rows); $i++) {
                if (preg_match($regexp, $rows[$i], $matches)) {
                    for ($j = 0; $j < count($columns->columns); $j++) {
        ...
        if (isset($id->ort)) {
            ...
            $rows = explode("\r\n", $contents);
            $columns = deffinum_get_aicc_columns($rows[0], 'course_element');
            $regexp = deffinum_forge_cols_regexp($columns->columns, '(.+)?,');
            for ($i = 1; $i < count($rows); $i++) {
                if (preg_match($regexp, $rows[$i], $matches)) {
        ...
        if (isset($id->pre)) {
            ...
            $rows = explode("\r\n", $contents);
            $columns = deffinum_get_aicc_columns($rows[0], 'structure_element');
            $regexp = deffinum_forge_cols_regexp($columns->columns, '(.+),');
            for ($i = 1; $i < count($rows); $i++) {
                if (preg_match($regexp, $rows[$i], $matches)) {
    ...
    $oldscoes = $DB->get_records('deffinum_scoes', array('deffinum' => $deffinum->id));
    $sortorder = 0;
    $launch = 0;
    if (isset($courses)) {
        foreach ($courses as $course) {
            ...
            if ($ss = $DB->get_record('deffinum_scoes', array('deffinum' => $deffinum->id,
                                                           'identifier' => $sco->identifier))) {
                $id = $ss->id;
                $sco->id = $id;
                $DB->update_record('deffinum_scoes', $sco);
                unset($oldscoes[$id]);
            } else {
                $id = $DB->insert_record('deffinum_scoes', $sco);
            }
            ...
            if (isset($course->elements)) {
                foreach ($course->elements as $element) {
                    ...
                    if (isset($element->file_name)) {
                        ...
                        $sco->deffinumtype = 'sco';
                    }
                    if ($oldscoid = deffinum_array_search('identifier', $sco->identifier, $oldscoes)) {
                        $sco->id = $oldscoid;
                        $DB->update_record('deffinum_scoes', $sco);
                        $id = $oldscoid;
                        $DB->delete_records('deffinum_scoes_data', array('scoid' => $oldscoid));
                    ...
                    if (!empty($id)) {
                        ...
                        if (isset($element->web_launch)) {
                            ...
                            $dataid = $DB->insert_record('deffinum_scoes_data', $scodata);
                        }
    ...
    if (!empty($oldscoes)) {
        foreach ($oldscoes as $oldsco) {
            deffinum_delete_tracks($deffinum->id, $oldsco->id);
            $DB->delete_records('deffinum_scoes', ['id' => $oldsco->id]);
        }
    }
...
/**
 * Given a deffinumid creates an AICC Session record to allow HACP
 *
 * @param int $deffinumid - id from deffinum table
 * @return string hacpsession
 */
function deffinum_aicc_get_hacp_session($deffinumid) {
    ...
    $hacpsession = $SESSION->deffinum;
    $hacpsession->deffinumid = $deffinumid;
 ...
 * @return mixed - false if invalid, otherwise returns record from deffinum_aicc_session table.
 */
function deffinum_aicc_confirm_hacp_session($hacpsession) {
    global $DB;
    $cfgdeffinum = get_config('deffinum');
    if (empty($cfgdeffinum->allowaicchacp)) {
        return false;
    }
    $time = time() - ($cfgdeffinum->aicchacptimeout * 60);
    $sql = "hacpsession = ? AND timemodified > ?";
    $hacpsession = $DB->get_record_select('deffinum_aicc_session', $sql, array($hacpsession, $time));
    if (!empty($hacpsession)) { // Update timemodified as this is still an active session - resets the timeout.
        ...
        $DB->update_record('deffinum_aicc_session', $hacpsession);
    }
    return $hacpsession;
}
 ...
 */
function deffinum_aicc_generate_simple_sco($deffinum) {
    global $DB;
    // Find the oldest one.
    $scos = $DB->get_records('deffinum_scoes', array('deffinum' => $deffinum->id), 'id');
    if (!empty($scos)) {
    ...
    $sco->deffinum = $deffinum->id;
    $sco->organization = '';
    $sco->title = $deffinum->name;
    $sco->parent = '/';
    // Add the HACP signal to the activity launcher.
    if (preg_match('/\?/', $deffinum->reference)) {
        $sco->launch = $deffinum->reference.'&CMI=HACP';
    } else {
        $sco->launch = $deffinum->reference.'?CMI=HACP';
    }
    $sco->deffinumtype = 'sco';
...
function get_deffinum_default (&$userdata, $deffinum, $scoid, $attempt, $mode) {
    ...
    $def['cmi.student_data.attempt_number'] = deffinum_isset($userdata, 'cmi.student_data.attempt_number');
    $def['cmi.student_data.mastery_score'] = deffinum_isset($userdata, 'mastery_score');
    $def['cmi.student_data.max_time_allowed'] = deffinum_isset($userdata, 'max_time_allowed');
    $def['cmi.student_data.time_limit_action'] = deffinum_isset($userdata, 'time_limit_action');
    $def['cmi.student_data.tries_during_lesson'] = deffinum_isset($userdata, 'cmi.student_data.tries_during_lesson');
