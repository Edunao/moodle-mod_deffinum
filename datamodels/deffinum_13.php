...
if ($scoes = $DB->get_records('deffinum_scoes', array('deffinum' => $deffinum->id), 'sortorder, id')) {
    ...
    foreach ($scoes as $sco) {
        ...
        $cmicommentsuser->{($sco->id)} = deffinum_reconstitute_array_element($deffinum->version, $userdata->{($sco->id)},
                                                                    'cmi.comments_from_learner', array());
        $cmicommentslms->{($sco->id)} = deffinum_reconstitute_array_element($deffinum->version, $userdata->{($sco->id)},
                                                                    'cmi.comments_from_lms', array());
    }
}

$deffinum->autocommit = ($deffinum->autocommit === "1") ? true : false;
$PAGE->requires->js_init_call('M.deffinum_api.init', array($def, $cmiobj, $cmiint, $cmicommentsuser, $cmicommentslms,
...
if (deffinum_debugging($deffinum)) {
    ...
    echo html_writer::script('AppendToLog("Moodle DEFFINUM 1.3 API Loaded, Activity: '.
